...
Settings::PortSettings Settings::getSettings() const
{
...
void Settings::showPortInfo(int idx)
{
    if (idx == -1) {
        return;
    }
    ...
    ui->serialNumLabel->setText(tr("Serial number: %1").arg(list.count() > 3 ? list.at(3) : tr(blankString)));
    ui->locationLabel->setText(tr("Location: %1").arg(list.count() > 4 ? list.at(4) : tr(blankString)));
    ui->vendorLabel->setText(tr("Vendor Identifier: %1").arg(list.count() > 5 ? list.at(5) : tr(blankString)));
    ui->productLabel->setText(tr("Product Identifier: %1").arg(list.count() > 6 ? list.at(6) : tr(blankString)));
}
...
void Settings::checkCustomBaudRatePolicy(int idx) {
...
void Settings::checkCustomDevicePathPolicy(int idx) {
    bool isCustomPath = !ui->serialPortBox->itemData(idx).isValid();
